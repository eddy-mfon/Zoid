...
type ShopContextValue = { bag: BagItem[]; count: number; total: number; addToBag: (product: Product, size?: string) => void; removeFromBag: (slug: string, size: string) => void; clearBag: () => void };
...
export function ShopProvider({ children }: { children: ReactNode }) {
  ...
  function addToBag(product: Product, size = product.sizes[0]) {
    setBag((items) => {
      const existing = items.find((item) => item.slug === product.slug && item.size === size);
      return existing ? items.map((item) => item === existing ? { ...item, quantity: item.quantity + 1 } : item) : [...items, { ...product, size, quantity: 1 }];
    });
    toast.success(`${product.name} added to your bag`, { description: `${size} · Curated for the ones still rising.` });
  }
